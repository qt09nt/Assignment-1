...
**1.Training scenario 1: apply Random Forest, KNN and Logistic Regresstion to transformed variables.**  
...
```{r}
cv_5=trainControl(method = "cv", number = 5, allowParallel = TRUE, summaryFunction = twoClassSummary , classProbs = TRUE)
```
...
conf_rf
#accuracy 79.3%%, sens 84%, spec 78%, NIR 75%, Kappa 0.5278  
library(pROC)
...
model_knn=train(income ~ age_stand+workclass+fnlwgt_logstand+education+marital_status_group+occupation+relationship+race+sex+native_region+cap_loss+cap_gain+weekly_hours,
                data = X_train_bal, trControl=cv_5, method="knn")
...
From the above plot of accuracy and number of neighbors, we can see that the accuracy still increases after training on the default number of ks. So we decide to increasing the number of k values to 10 to try to locate a better model.


```{r}
model_knn=train(income ~ age_stand+workclass+fnlwgt_logstand+education+marital_status_group+occupation+relationship+race+sex+native_region+cap_loss+cap_gain+weekly_hours,
                data = X_train_bal, trControl=cv_5, method="knn", tuneLength = 10)
plot(model_knn)
```

Now we can see the accuracy starts to decrease once the number of neighbors reached about 13.
...
conf_knn
#accuracy 77%, sens 83%, spec 75%, NIR 75%
...
**Logistic Regression Model on unstandardized age and fnl_wgt.**
...
**Logistic Regression Model on unstandardized age without fnl_wgt.**
...
#accuracy 80%, sens 81.8%, spec 80%, NIR 75%
